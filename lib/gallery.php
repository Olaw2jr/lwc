...
function gallery($attr) {
  ...
  $output = '<div class="lwc-gallery gallery-' . $id . '-' . $unique . '">';
  $i = 0;
  foreach ($attachments as $id => $attachment) {
    switch($link) {
      case 'file':
        ...
        break;
      case 'none':
        $image = wp_get_attachment_image($id, $size, false, ['class' => 'img-fluid img-thumbnail']);
        break;
      default:
...
function attachment_link_class($html) {
  $html = str_replace('<a', '<a class="lightbox thumbnail img-thumbnail" href="#"', $html);
  return $html;
}
add_filter('wp_get_attachment_link', __NAMESPACE__ . '\\attachment_link_class', 10, 1);

/**
 * Stupid me trying to add a class to image thing of which 
 */